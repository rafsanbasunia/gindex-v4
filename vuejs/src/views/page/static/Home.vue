...
<section :class="ismobile ? logged ? 'section mx-0 px-0' : 'section mx-0 px-0' : logged ? 'section mx-0 px-0' : 'section mx-0 px-0' " :style="logged ? '' : 'min-width:100%;background-image: url('+backurl+');background-size:cover;min-width:100%;box-shadow:inset 0 0 0 2000px rgba(0, 0, 0, 0.5);' ">
    <nav v-if="!logged" class="navbar home-navbar">
        ...
        <div v-if="!ismobile" class="navbar-end">
            <a class="navbar-item button" title="Login" v-if="!logged" @click="gotoPage('/', 'login')">
                <span>Login</span>
                <span class="icon">
        ...
        <div class="column is-full mx-0 px-0">
            ...
            <div v-if="logged && !netflix" class="tags-has-addons mt-3">
                <div class="container has-text-white is-fluid">
                    <h1 class="title has-text-white has-text-centered">Hey There, Whaat up? <p class="has-text-info is-italic has-text-weight-bold is-family-monospace">{{ user.name }}</p>
                    </h1>
...
export default {
    ...
    methods: {
        gotoPage(url, cmd) {
            this.$ga.event({
                eventCategory: "Page Navigation",
                eventAction: url + " - " + this.siteName,
                eventLabel: "Home"
            })
            if (cmd) {
                this.$router.push({
                    path: '/' + this.currgd.id + ':' + cmd + url
                })
            } else {
                this.$router.push({
                    path: '/' + this.currgd.id + ':' + url
                })
            }
        },
        assignUserInfo() {
            ...
            if (userData.isThere) {
                if (userData.type == "hybrid") {
                    ...
                    this.$meta().refresh
                    this.$ga.event({
                        eventCategory: "User Initialized",
                        eventAction: "Hybrid - " + this.siteName,
                        eventLabel: "Home",
                        nonInteraction: true
                    })
                    ...
                    this.$ga.event({
                        eventCategory: "User Initialized",
                        eventAction: "Normal - " + this.siteName,
                        eventLabel: "Home",
                        nonInteraction: true
                    })
                }
                ...
                this.$http.post(window.apiRoutes.checkEmail, {
                    ...
                    if (response.data.auth && response.data.user && response.data.status == "User Present & Verified") {
                        ...
                        this.$ga.event({
                            ...
                            eventAction: "User Present & Verified" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        ...
                        this.$ga.event({
                            eventCategory: "Email Verification",
                            eventAction: "User Present & Not Verified" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        this.$router.push({
                            name: 'otp',
                        ...
                        this.$ga.event({
                            eventCategory: "Email Verification",
                            eventAction: "User Not Present" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        this.$router.push({
                            name: 'request',
                        ...
                        this.$ga.event({
                            eventCategory: "Email Verification",
                            eventAction: "Pending Confirmation from Admins" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        this.$router.push({
                            name: 'results',
                            params: {
                                ...
                                noredirect: true,
                                success: true,
                                data: "You Are Currently Pending Confirmation from Admins. Please Wait till they Accept Your Request."
                            }
                        })
                        ...
                        this.$ga.event({
                            eventCategory: "Email Verification",
                            eventAction: "Spammed User" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        this.$router.push({
                            name: 'results',
                            params: {
                                ...
                                noredirect: true,
                                success: false,
                                data: "You are being Added to Our Spam List for Violations. Please Contact Admins for Help."
                            }
                        })
                    } else {
                        this.loading = false;
                        ...
                        this.$ga.event({
                            eventCategory: "Email Verification",
                            eventAction: "Network Error" + " - " + this.siteName,
                            eventLabel: "Home"
                        })
                        this.$router.push({
                            name: 'results',
                            params: {
                                ...
                                noredirect: true,
                                success: false,
                                data: "There's Some Error With Your Network. Please Try Again Later."
                            }
                        })
                    }
                }).catch(error => {
